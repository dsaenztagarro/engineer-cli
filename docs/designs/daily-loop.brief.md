...
> **Historical / superseded — do not read this for current-state guidance.** This is the pre-lifecycle brief that drove the first wave of daily-loop screens (timer, notes, activities, review, command palette). All of them have shipped, so every sentence below is written in the wrong tense: it asks for work that is long since done. It also predates the [input lifecycle](briefs/README.md), under which a brief is deleted once the surface it asked for is live — whether it should retire too is [#187](https://github.com/dsaenztagarro/engineer-cli/issues/187).
>
> The durable references are the design kit ([`README.md`](README.md)), the `.dc.html` boards beside this file, and the decision records in [`../architecture/decisions/`](../architecture/decisions/) — in particular [ADR 0002](../architecture/decisions/0002-sterling-not-a-replica.md), which carries the governing principle this brief opens with. Two anchors named below are **retired**: `books.html` and `terminal-tokens.css` no longer exist; the live style anchor is [`design-system.dc.html`](design-system.dc.html).
