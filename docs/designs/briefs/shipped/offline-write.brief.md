...
**Produces:** the terminal states and headless surfaces of writing offline — the timer as a genuine **local clock** that controls locally and reconciles on reconnect, a persisted **write queue** every mutation rides, and the **divergence/reconcile** moments when local and server disagree. Extend `../../design-system.dc.html` (the style anchor); the timer states are the natural extension of `../../timer.dc.html`, and the queue/reconcile surfaces have their own board (`offline-write.dc.html`, mirroring how `timer.dc.html` is one board per area).
**Status:** **shipped.** (read half #91 → v0.7.0; the write half — foundation + local clock EPIC #98 #99–#107 → v0.8.0; the long tail + Queue inspector #108–#112 → v0.10.0)
The whole offline-write mechanism is live: the timer is a genuine local clock (`src/timer_clock.rs`), every mutation rides a persisted optimistic write queue (`src/queue/`), and a `422` on replay becomes a resolvable divergence rather than a silent drop. Foundation + the train case landed first (#99–#107 → v0.8.0), then the long tail brought every remaining write path onto the one queue and gave it its own face — the Queue inspector board (#108–#112 → v0.10.0). Tracking issue **#96** (follow-up to **#81**) is closed with EPIC #98. This brief is kept as the module record; the open server contract in §5 is now **RESOLVED** (ADR 0036) and the §8 phases are all shipped (see the notes there). Residual, recorded honestly: a set of verbs stays **live-only** by design — a review rating, inbox triage, source connect, note/segment hard deletes, timer reclaim, and the presence heartbeat each refuse offline rather than synthesize a lie (the full #111 inventory); the headless suite's richer exit-code spread was deferred (#123); and the ⏎-opens-divergence deep-link from the header chip was narrowed to the Queue inspector list and the Timer reconcile panel (#112 — the TUI header stays a static read).
...
- **Shipped — the whole write mechanism.** The persisted intent queue and its reconciler are live as a `src/queue/` module: `intent.rs` (the serde-tagged `IntentKind` write vocabulary), `store.rs` (the durable, writer-locked `queue.json` in the XDG state dir), `client.rs` (the `QueuedClient` seam every write rides), `replay.rs` (the per-stream FIFO drain, the provisional→real id-map, and divergence gating), `resolve.rs` (the edit / drop / skip reconcile choices), `fold.rs` (provisional rows folded into reads without a second ledger), and `view.rs` (the one `#  INTENT  TARGET  AGE  STATE` shape the board and the CLI table both render). The timer became a controlling local clock in `src/timer_clock.rs`; the headless twin is `src/queue_cli.rs` (`engineer queue`), and the Queue inspector board is `src/app/screens/queue.rs`. `queued` / `diverged` surface in `--json`, the plain status line, and exit codes exactly as `stale` already does. *(The pre-build orientation below is kept as the record of where the module started — it named the queue as net-new, which it is no longer.)*
...
> **RESOLVED (dsaenztagarro/engineer#806 → PR #809 → ADR 0036, consumed in #107).** Both halves of the contract shipped. **Idempotency under replay:** the server takes an `Idempotency-Key` on its opt-in mutation set (timer start, segment- and activity-create), so a queued write re-sent after a lost ack dedupes instead of double-writing — the CLI stamps the key when it enqueues and replays it verbatim. **A structured conflict the client can render:** the server returns machine-readable coded conflicts the reconcile flow turns into a choice, so a `422` on replay surfaces the divergence face (edit / drop / skip) rather than a generic error. Consumed in #107; the client mechanics that ride it are #106/#109. The original gap analysis is kept below as the record of what drove the design.
...
1. **Foundation — the persisted intent queue + reconciler. — shipped (EPIC #98 Phase 1, #99–#107 → v0.8.0).** A small module sibling to `timer_cache.rs`: a durable, ordered log of pending write intents in the XDG state dir, a replay-on-reconnect pass keyed on `ApiError::Transport`, and the `queued` / `diverged` state vocabulary surfaced in `--json` + plain + exit codes. Shared by both client surfaces. This is shared machinery every later phase builds on — the "shared vocabulary first" the epic orders foundation before surfaces. *Shipped as `src/queue/` (`store`/`intent`/`replay`) with the `queued`/`diverged` vocabulary in `--json`, plain, and exit codes.*
2. **Phase A — the timer as a local clock (the train case; highest value, standalone). — shipped (EPIC #98 Phase 2, #99–#107 → v0.8.0).** Turn `live_elapsed` and the poll/heartbeat loop into a controlling local clock; make timer start/pause/resume/stop/bind/discard act locally and enqueue; design the reconcile-on-reconnect and the divergence moment for the timer specifically (a session started elsewhere, a drifted clock). Touches the 10 timer methods + `create_segment`/`delete_segment` (stop writes a segment). Ships the headline "study on trains" value on its own. *Shipped: `src/timer_clock.rs` is the controlling clock, the timer verbs enqueue offline, and the reconcile-on-reconnect + divergence moment ship as the Timer reconcile panel (#106/#109).*
3. **Phase B — segment & activity writes on the same queue. — shipped (#108, #110 → v0.10.0; parts rode the surface epics).** `engineer log` (create_activity + create_segment), `engineer week plan`, the activities table and segment audit — the same queue-and-reconcile shape, now that the timer proved it. *Shipped: `engineer log` offline (#108), the activity-lifecycle + `engineer plan add` mop-up (#110), with the Week board plan writes, targets, and note writes already made queue-aware in their own epics (#115/#117, #121, #123).*
4. **Phase C — the long tail. — shipped (#111 → v0.10.0).** notes / targets / books / review / inbox writes ride the identical mechanism. Framed here (not smuggled into each module brief) precisely so it is one queue, not five. *Shipped: notes / books / headless targets and the last un-queued gestures all ride the one `QueuedClient` (#111), and the queue finally got its own face — the Queue inspector board + `engineer queue` verb set (#112).*

Gating note — **resolved.** The server contract in §5 shipped (ADR 0036), so honest divergence resolution (job 4) landed in-band (#106/#107/#109) and no phase was left blocked on the server. The only writes that stay live-only do so because an offline synthesis would *lie* (a rating's next-due schedule, a destructive delete, a reclaim's idle verdict), not because the contract is missing — the full inventory is #111.
